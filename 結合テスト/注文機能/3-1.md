...
- DBの状態変化を確認するため、テスト前後の `products` テーブルの在庫数、`orders` テーブル、`order_items` テーブルのレコード数や内容を確認します。
...
- 期待結果:
  ...
  3. DB状態変化:
     ...
     - `order_items` テーブルに1件レコードが追加され、上記の `orders` レコードに関連付き、商品情報(商品A, 500)、数量(3)が正しく登録されていること。
